...
public class TextSourceParser : ITextSourceParser
{
    ...
    public Result<ITextSource> Parse(string text)
    {
        ...
        if (list.All(i => !i.IsVariable))
        {
            var joinedString = string.Join(string.Empty, list.Select(i => i.Value));
            return Result.Ok<ITextSource>(new StringTextSource(joinedString));
        }

        if (list.Count == 1 && list[0].IsVariable)
