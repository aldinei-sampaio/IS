...
public class HierarchyTester
{
    ...
    private void ConfigureMove(bool forward, IBlock block, params INode[] nodes)
    {
        ...
        if (forward)
            A.CallTo(() => blockNavigator.MoveNextAsync(block, navigationContext)).ReturnsNextFromSequence(nodes);
        else
            A.CallTo(() => blockNavigator.MovePreviousAsync(block, navigationContext)).ReturnsNextFromSequence(nodes);
    }
